...
struct voxel_coord_kernel {
  ...
  __host__ __device__ Eigen::Vector3i operator()(
...
struct voxel_accumulation_kernel {
  ...
  __device__ void operator()(const Tuple& tuple) const {
    ...
    const thrust::pair<Eigen::Vector3i, int>* buckets =
        thrust::raw_pointer_cast(buckets_ptr);
    ...
    for (int i = 0; i < max_bucket_scan_count; i++) {
      ...
      if (thrust::get<0>(buckets[bucket_index]) == coord) {
        ...
        int* num_points =
            thrust::raw_pointer_cast(num_points_ptr) + voxel_index;
        Eigen::Vector3f* voxel_mean =
            thrust::raw_pointer_cast(voxel_means_ptr) + voxel_index;
        Eigen::Matrix3f* voxel_cov =
            thrust::raw_pointer_cast(voxel_covs_ptr) + voxel_index;
...
class GaussianVoxelMap {
 public:
  GaussianVoxelMap(float resolution,
                   int init_num_buckets = 8192 * 4,
  ...
  void create_voxelmap(
    ...
    thrust::fill(
        voxel_means.begin(), voxel_means.end(), Eigen::Vector3f::Zero().eval());
    thrust::fill(
        voxel_covs.begin(), voxel_covs.end(), Eigen::Matrix3f::Zero().eval());

    thrust::for_each(
        thrust::make_zip_iterator(thrust::make_tuple(
            voxel_coords.begin(), points.begin(), covariances.begin())),
        thrust::make_zip_iterator(thrust::make_tuple(
            voxel_coords.end(), points.end(), covariances.end())),
        voxel_accumulation_kernel(voxel_resolution,
                                  max_bucket_scan_count,
                                  buckets,
                                  num_points,
                                  voxel_means,
                                  voxel_covs));
    ...
    thrust::for_each(
        thrust::make_zip_iterator(thrust::make_tuple(
            num_points.begin(), voxel_means.begin(), voxel_covs.begin())),
        thrust::make_zip_iterator(thrust::make_tuple(
            num_points.end(), voxel_means.end(), voxel_covs.end())),
        voxel_normalization_kernel());
  }

 private:
  void create_hashtable(
      const thrust::device_vector<Eigen::Vector3i>& voxel_coords) {
    thrust::host_vector<Eigen::Vector3i,
                        Eigen::aligned_allocator<Eigen::Vector3i>>
        coords = voxel_coords;

    num_voxels = 0;
    thrust::host_vector<thrust::pair<Eigen::Vector3i, int>> buckets(
        init_num_buckets);
    thrust::fill(buckets.begin(),
                 buckets.end(),
                 thrust::make_pair(Eigen::Vector3i(0, 0, 0), -1));
  ...
  const thrust::pair<Eigen::Vector3i, int>* find(
      const Eigen::Vector3i& coord,
      const thrust::host_vector<thrust::pair<Eigen::Vector3i, int>>& buckets)
      const {
    uint64_t hash = vector3i_hash(coord);
    for (int i = 0; i < max_bucket_scan_count; i++) {
  ...
  bool insert(
      const thrust::pair<Eigen::Vector3i, int>& voxel,
      thrust::host_vector<thrust::pair<Eigen::Vector3i, int>>& buckets) {
  ...
  void rehash(
    ...
    thrust::host_vector<thrust::pair<Eigen::Vector3i, int>> new_buckets(
        new_num_buckets);
    thrust::fill(new_buckets.begin(),
                 new_buckets.end(),
                 thrust::make_pair(Eigen::Vector3i(0, 0, 0), -1));
