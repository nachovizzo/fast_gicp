...
struct compute_derivatives_kernel {
  ...
  compute_derivatives_kernel(const GaussianVoxelMap& voxelmap,
  ...
  __host__ __device__ int lookup_voxel(const Eigen::Vector3f& x) const {
    ...
    for (int i = 0; i < max_bucket_scan_count; i++) {
      ...
      const thrust::pair<Eigen::Vector3i, int>& bucket =
          thrust::raw_pointer_cast(buckets_ptr)[bucket_index];
  ...
  __host__ __device__ Eigen::Matrix3f skew_symmetric(
  ...
  __host__ __device__ void operator()(Tuple tuple) const {
    ...
    int num_points =
        thrust::raw_pointer_cast(voxel_num_points_ptr)[voxel_index];
    const Eigen::Vector3f& mean_B =
        thrust::raw_pointer_cast(voxel_means_ptr)[voxel_index];
    const Eigen::Matrix3f& cov_B =
...
void compute_derivatives(
  ...
  thrust::for_each(
      thrust::make_zip_iterator(thrust::make_tuple(
          src_points.begin(), src_covs.begin(), losses.begin(), Js.begin())),
      thrust::make_zip_iterator(thrust::make_tuple(
          src_points.end(), src_covs.end(), losses.end(), Js.end())),
      compute_derivatives_kernel(voxelmap, x));

  // erase invalid points
  losses.erase(thrust::remove_if(losses.begin(), losses.end(), is_nan_kernel()),
               losses.end());
