...
struct covariance_regularization_svd {
  __host__ __device__ void operator()(Eigen::Matrix3f& cov) const {
    ...
    // cov = eig.eigenvectors() * values.asDiagonal() *
...
static void covariance_estimation(
    const thrust::device_vector<Eigen::Vector3f>& points,
    int k,
    const thrust::device_vector<int>& k_neighbors,
    thrust::device_vector<Eigen::Matrix3f>& covariances,
  ...
  thrust::for_each(
      d_indices.begin(),
      d_indices.end(),
      covariance_estimation_kernel(points, k, k_neighbors, covariances));

  switch (method) {
    default:
      std::cerr
          << "unimplemented covariance regularization method was selected!!"
          << std::endl;
      abort();
    case PLANE:
      thrust::for_each(covariances.begin(),
                       covariances.end(),
                       covariance_regularization_svd());
      break;
    case FROBENIUS:
      thrust::for_each(covariances.begin(),
                       covariances.end(),
                       covariance_regularization_frobenius());
      break;
  }
}
}  // namespace fast_gicp
