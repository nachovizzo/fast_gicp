...
struct neighborsearch_kernel {
  ...
  __host__ __device__ void operator()(const Tuple& idx_x) const {
    ...
    thrust::pair<float, int>* k_neighbors =
        thrust::raw_pointer_cast(k_neighbors_ptr) + idx * k;

    // priority queue
    struct compare_type {
      bool operator()(const thrust::pair<float, int>& lhs,
                      const thrust::pair<float, int>& rhs) {
    ...
    typedef nvbio::
        priority_queue<thrust::pair<float, int>, vector_type, compare_type>
            queue_type;
...
struct sorting_kernel {
  sorting_kernel(int k,
                 thrust::device_vector<thrust::pair<float, int>>& k_neighbors)
      : k(k), k_neighbors_ptr(k_neighbors.data()) {}
...
static void brute_force_knn_search(
  ...
  auto first = thrust::make_zip_iterator(
      thrust::make_tuple(d_indices.begin(), source.begin()));
  auto last = thrust::make_zip_iterator(
      thrust::make_tuple(d_indices.end(), source.end()));
  ...
  if (do_sort) {
    thrust::for_each(
        d_indices.begin(), d_indices.end(), sorting_kernel(k, k_neighbors));
  }
}
