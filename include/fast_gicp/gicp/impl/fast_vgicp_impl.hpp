...
template <typename PointSource, typename PointTarget>
void FastVGICP<PointSource, PointTarget>::setRegularizationMethod(
    RegularizationMethod method) {
  regularization_method_ = method;
}

template <typename PointSource, typename PointTarget>
void FastVGICP<PointSource, PointTarget>::setNeighborSearchMethod(
    NeighborSearchMethod method) {
  search_method_ = method;
}

template <typename PointSource, typename PointTarget>
void FastVGICP<PointSource, PointTarget>::setVoxelAccumulationMode(
    VoxelAccumulationMode mode) {
  voxel_mode_ = mode;
}
...
template <typename PointSource, typename PointTarget>
void FastVGICP<PointSource, PointTarget>::setInputSource(
...
template <typename PointSource, typename PointTarget>
void FastVGICP<PointSource, PointTarget>::setInputTarget(
...
template <typename PointSource, typename PointTarget>
void FastVGICP<PointSource, PointTarget>::create_voxelmap(
    const PointCloudTargetConstPtr& cloud) {
  voxels.clear();
...
template <typename PointSource, typename PointTarget>
void FastVGICP<PointSource, PointTarget>::computeTransformation(
...
FastVGICP<PointSource, PointTarget>::neighbor_offsets() const {
...
template <typename PointSource, typename PointTarget>
Eigen::Vector3i FastVGICP<PointSource, PointTarget>::voxel_coord(
    const Eigen::Vector4f& x) const {
  return (x.array() / voxel_resolution_ - 0.5)
      .floor()
      .template cast<int>()
      .template head<3>();
}
...
template <typename PointSource, typename PointTarget>
Eigen::Vector4f FastVGICP<PointSource, PointTarget>::voxel_origin(
    const Eigen::Vector3i& coord) const {
  Eigen::Vector3f origin =
      (coord.cast<float>().array() + 0.5) * voxel_resolution_;
  return Eigen::Vector4f(origin[0], origin[1], origin[2], 1.0f);
}

template <typename PointSource, typename PointTarget>
GaussianVoxel::Ptr FastVGICP<PointSource, PointTarget>::lookup_voxel(
    const Eigen::Vector3i& x) const {
...
template <typename PointSource, typename PointTarget>
bool FastVGICP<PointSource, PointTarget>::is_converged(
...
template <typename PointSource, typename PointTarget>
Eigen::VectorXf FastVGICP<PointSource, PointTarget>::loss_ls(
  ...
  std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>>
      losses(input_->size() * offsets.size());
...
bool FastVGICP<PointSource, PointTarget>::calculate_covariances(
