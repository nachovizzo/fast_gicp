...
template <typename PointSource, typename PointTarget>
void FastGICPMultiPoints<PointSource, PointTarget>::setRotationEpsilon(
    double eps) {
...
template <typename PointSource, typename PointTarget>
void FastGICPMultiPoints<PointSource, PointTarget>::setCorrespondenceRandomness(
    int k) {
  k_correspondences_ = k;
}

template <typename PointSource, typename PointTarget>
void FastGICPMultiPoints<PointSource, PointTarget>::setRegularizationMethod(
    RegularizationMethod method) {
  regularization_method_ = method;
}

template <typename PointSource, typename PointTarget>
void FastGICPMultiPoints<PointSource, PointTarget>::setInputSource(
...
template <typename PointSource, typename PointTarget>
void FastGICPMultiPoints<PointSource, PointTarget>::setInputTarget(
...
template <typename PointSource, typename PointTarget>
void FastGICPMultiPoints<PointSource, PointTarget>::computeTransformation(
  ...
  for (int i = 0; i < max_iterations_; i++) {
    ...
    x0.head<3>() =
        (Sophus::SO3f::exp(-delta.head<3>()) * Sophus::SO3f::exp(x0.head<3>()))
            .log();
...
template <typename PointSource, typename PointTarget>
bool FastGICPMultiPoints<PointSource, PointTarget>::is_converged(
...
template <typename PointSource, typename PointTarget>
void FastGICPMultiPoints<PointSource, PointTarget>::update_correspondences(
...
#pragma omp parallel for num_threads(num_threads_)
  for (int i = 0; i < input_->size(); i++) {
    ...
    target_kdtree.radiusSearch(
        pt, neighbor_search_radius_, k_indices, k_sq_dists);
...
template <typename PointSource, typename PointTarget>
Eigen::VectorXf FastGICPMultiPoints<PointSource, PointTarget>::loss_ls(
...
bool FastGICPMultiPoints<PointSource, PointTarget>::calculate_covariances(
  ...
  for (int i = 0; i < cloud->size(); i++) {
    ...
    if (regularization_method_ == FROBENIUS) {
      ...
      covariances[i].template block<3, 3>(0, 0) =
          (C_inv / C_inv.norm()).inverse().cast<float>();
    } else {
