...
FastVGICPCudaCore ::~FastVGICPCudaCore() { cublasDestroy(cublas_handle); }
...
void FastVGICPCudaCore::set_source_cloud(
    const std::vector<Eigen::Vector3f,
                      Eigen::aligned_allocator<Eigen::Vector3f>>& cloud) {
  thrust::host_vector<Eigen::Vector3f,
                      Eigen::aligned_allocator<Eigen::Vector3f>>
      points(cloud.begin(), cloud.end());
  source_points.reset(new Points(points));
}

void FastVGICPCudaCore::set_target_cloud(
    const std::vector<Eigen::Vector3f,
                      Eigen::aligned_allocator<Eigen::Vector3f>>& cloud) {
  thrust::host_vector<Eigen::Vector3f,
                      Eigen::aligned_allocator<Eigen::Vector3f>>
...
void FastVGICPCudaCore::set_source_neighbors(
    int k, const std::vector<int>& neighbors) {
...
void FastVGICPCudaCore::set_target_neighbors(
    int k, const std::vector<int>& neighbors) {
...
void FastVGICPCudaCore::find_source_neighbors(int k) {
  ...
  thrust::transform(k_neighbors.begin(),
                    k_neighbors.end(),
...
void FastVGICPCudaCore::calculate_source_covariances(
  ...
  if (!source_covariances) {
    source_covariances.reset(
        new thrust::device_vector<Eigen::Matrix3f>(source_points->size()));
  }
  covariance_estimation(
      *source_points, k, *source_neighbors, *source_covariances, method);
}
...
void FastVGICPCudaCore::calculate_target_covariances(
  ...
  if (!target_covariances) {
    target_covariances.reset(
        new thrust::device_vector<Eigen::Matrix3f>(target_points->size()));
  }
  covariance_estimation(
      *target_points, k, *target_neighbors, *target_covariances, method);
}
...
bool FastVGICPCudaCore::is_converged(
...
bool FastVGICPCudaCore::optimize(const Eigen::Isometry3f& initial_guess,
  ...
  thrust::device_vector<Eigen::Matrix<float, 3, 6, Eigen::RowMajor>>
      Js;  // RowMajor 3Nx6 -> ColMajor 6x3N
  ...
  for (int i = 0; i < max_iterations; i++) {
    compute_derivatives(
        *source_points, *source_covariances, *voxelmap, x0, losses, Js);
    ...
    cublasSgemm(cublas_handle,
                ...
                6,
                &beta,
                thrust::raw_pointer_cast(JJ_ptr),
                6);
    ...
    cublasSgemv(cublas_handle,
                ...
                1,
                &beta,
                thrust::raw_pointer_cast(J_loss_ptr),
                1);

    Eigen::Matrix<float, 6, 6> JJ;
    cublasGetMatrix(
        6, 6, sizeof(float), thrust::raw_pointer_cast(JJ_ptr), 6, JJ.data(), 6);
