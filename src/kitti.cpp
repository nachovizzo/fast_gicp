 ...
 public:
  KittiLoader(const std::string& dataset_path) : dataset_path(dataset_path) {
    for (num_frames = 0;; num_frames++) {
      std::string filename =
          (boost::format("%s/%06d.bin") % dataset_path % num_frames).str();
  ...
  pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud(size_t i) const {
    std::string filename =
        (boost::format("%s/%06d.bin") % dataset_path % i).str();
...
int main(int argc, char** argv) {
  if (argc < 2) {
    std::cout << "usage: gicp_kitti /your/kitti/path/sequences/00/velodyne"
              << std::endl;
    return 0;
  }
  ...
  // registration method
  // you should fine-tune hyper-parameters (e.g., voxel resolution, max
  // correspondence distance) for the best result
  ...
  // for calculating FPS
  boost::circular_buffer<std::chrono::high_resolution_clock::time_point> stamps(
      30);
  ...
  for (int i = 1; i < kitti.size(); i++) {
    ...
    trajectory->push_back(
        pcl::PointXYZ(poses[i](0, 3), poses[i](1, 3), poses[i](2, 3)));
    vis.updatePointCloud<pcl::PointXYZ>(
        trajectory,
        pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ>(
            trajectory, 255.0, 0.0, 0.0),
        "trajectory");
    vis.spinOnce();
  }
