...
void test_pcl(Registration& reg,
  ...
  double multi =
      std::chrono::duration_cast<std::chrono::nanoseconds>(t2 - t1).count() /
      1e6;
  std::cout << "100times:" << multi
            << "[msec] fitness_score:" << reg.getFitnessScore() << std::endl;
}

// benchmark for fast_gicp registration methods
template <typename Registration>
void test(Registration& reg,
          const pcl::PointCloud<pcl::PointXYZ>::ConstPtr& target,
          const pcl::PointCloud<pcl::PointXYZ>::ConstPtr& source) {
  pcl::PointCloud<pcl::PointXYZ>::Ptr aligned(
      new pcl::PointCloud<pcl::PointXYZ>);
  ...
  // single run
  auto t1 = std::chrono::high_resolution_clock::now();
  // fast_gicp reuses calculated covariances if an input cloud is the same as
  // the previous one to prevent this for benchmarking, force clear source and
  // target clouds
  reg.clearTarget();
  reg.clearSource();
  reg.setInputTarget(target);
  ...
  // you can reuse the covariances of a source point cloud in the next
  // registration
  t1 = std::chrono::high_resolution_clock::now();
...
int main(int argc, char** argv) {
  ...
  std::cout << "target:" << target_cloud->size()
            << "[pts] source:" << target_cloud->size() << "[pts]" << std::endl;
  ...
  // vgicp_cuda uses CPU-based parallel KDTree in covariance estimation by
  // default on a modern CPU, it is faster than GPU_BRUTEFORCE
  ...
  // this would be a good choice if your PC has a weak CPU and a strong GPU
  // (e.g., NVIDIA Jetson)
